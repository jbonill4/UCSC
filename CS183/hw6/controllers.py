...
def get_posts():
    ...
    #Steps: get post data and add it
    #		trace through the list and add name, email, and get_name_from email
    # postL = db(db.post).select().as_list() #postList

    # for post in postL:
    # 	email = post.get("email")
    # 	name = get_name_from_email(email)
    # 	post['author'] = name
    # return dict(posts=postL)

    #REORDERING THE POSTSSS
    finalList = []
...
def save_post():
    ...
    if id == None: 
    	id = db.post.insert(content=content, is_reply=is_reply) #ADDED
    else:  #REMOVABLE
    	db(db.post.id == id).update(content=content, is_reply=is_reply) #ADDED
